...
function UIClass()
{
  ...
  {
    var html = HtmlService.createHtmlOutput().setWidth(550).setHeight(300);
...
width=60 style="padding-left:10px;padding-right:15px;"><p><br>' + tip_html + "</p></div>";
  ...
  //
  // Display a generic UI for messages. Shown with a Flubaroo logo.
  this.showSidebarMessage = function(title, msg)
  {
    if (this.isOn())
      {
        var html = UI.createSidebarMessage(title, msg);
        SpreadsheetApp.getUi()
                      .showSidebar(html);
      }
  }
  ...
  {
    var html = HtmlService.createHtmlOutput().setWidth(510).setHeight(155);
  ...
  this.createSidebarMessage = function(title, msg)
  {
  ...
  this.advancedOptions = function()
  {
    ...
    */
    ...
    var rates = new Array("0.50", "0.55", "0.60", "0.65", "0.70", "0.75", "0.80", "0.85", "0.90", "0.95");
    var selected = [];
