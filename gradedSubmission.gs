  ...
  for (var q=this.getFirstQuestion(); q != null; q = this.getNextQuestion(q))
    {
      ...
      switch (gopt)
        ...
        default:
            ...
            // Typically here we just grade the submission against the answer key.
            // But we also check for the case that the answer key is a formula to be
            // inserted into the sheet (for dynamic scoring), in which case we just
            ...
            if (anskey_is_formula)
              {
                // write out the formula that will determine the score
                grade_val = ak_val;
              }
            else
              {
  ...
  if (typeof key === 'string')
    {
      if ((key.search(ANSKEY_OPERATOR_CASE_SENSITIVE) == 0) && (key.length > 4))
        ...
        {
          key_set = this.processCheckBoxInKey(key);
          found_cb = true;
        }
      else
        {
...
{
  var key_vals = key.substr(4); // skip past '%cb ';
  ...
  if (output_row_type == GRADES_OUTPUT_ROW_TYPE_GRADED_VALS)
    {
      ...
      // copy over user-created formulas, if any, from the Grades sheet.
      // we'll want to write these back out.
      ...
      if (this.grading_options[i] != GRADING_OPT_STUD_ID)
        {
          if (output_row_type == GRADES_OUTPUT_ROW_TYPE_QUESTIONS_HEADER)
            {
              //Debug.info("createRowForGradesSheet() - About to add question summary [i] = " + i);
              output_row.push(createQuestionSummary(data_vals[i]));
            }
          else
            {
