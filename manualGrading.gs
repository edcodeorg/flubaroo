...
function mgrInitGlobals()
{
  ...
  var hidden_first_row_vals = sheet_gbl.getRange(1, 1, 1, 2).getValues();

  // number of student identifiers is recorded in the hidden row (B1)
  num_stud_identifiers_gbl = Number(sheet_gbl.getRange(1, 2, 1, 1).getValue());

  // number of counted submissions will be read from the summary of grades at the top of the sheet
  var r = "B" + GRADES_SUMMARY_COUNTED_SUBM_ROW_NUM + ":" + "B" + GRADES_SUMMARY_COUNTED_SUBM_ROW_NUM; // i.e. B5:B5
  num_graded_subm_gbl = Number(sheet_gbl.getRange(r).getValue());
  ...
  var cell_a1_range = sheet_gbl.getRange(1, 1, 1, 1);
  hidden_row_num_start_gbl = cell_a1_range.getValue();

  // get a valid range of columns
  min_col_gbl = 1 + num_stud_identifiers_gbl + gbl_num_metrics_cols + 1;
  max_col_gbl = sheet_gbl.getLastColumn();
...
function mgrGetNextManuallyGradedSubmission(current_col_num, current_row_num, direction)
{
  ...
  if (direction > 0)
    {
      // step forwards
      if (current_row_num < grade_values_end_row_gbl)
        {
    ...
    {
      if (current_row_num > (gbl_grades_start_row_num + 1))
        {
          data.row_num = current_row_num - 1;
        }
    }
...
function mgrSetTeacherComment(col_num, row_num, comment)
{
  ...
  return sheet_gbl.getRange(teacher_comment_row, col_num, 1, 1).setValue(comment);
}
